...
public class StoringController {
    ...
    @PostMapping("/storingList")
    public List<StoringListDTO> storingList(@RequestBody ListInfo input) {
        HashMap<String, String> searchInfo = new HashMap<>();

        searchInfo.put("cust_cd", input.getCust_cd());
        searchInfo.put("purc_in_dt_1", input.getDt_1());
        searchInfo.put("purc_in_dt_2", input.getDt_2());
    ...
    @PostMapping("/storingDetailList")
    public List<StoringListDTO> storingDetailList(@RequestBody ListInfo input) {
        HashMap<String, String> searchInfo = new HashMap<>();
    ...
    @PostMapping("/storingInsert")
    public InsertResult insertStoring(@RequestParam(name = "jwt") String jwt, @RequestBody InsertInfo input) {
        ...
        if(searchResult.isEmpty()) { // 없으면 새로 입고와 입고 상세 데이터를 생성
            purc_in_no = storingService.purc_in_no_Cal(jwtInfo.get(1)); // 입고 번호 생성

            storedData = new StoringDTO(jwtInfo.get(1), jwtInfo.get(2), purc_in_no, input.getCust_cd(), jwtInfo.get(0));
            storedDetailData = new StoringDetailDTO(jwtInfo.get(1), purc_in_no, input.getItem_cd(), input.getQty(), input.getStor_cd(), input.getLoca_cd(), jwtInfo.get(0));
